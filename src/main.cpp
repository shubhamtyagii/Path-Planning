      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			vector<double> next_x_30= getXY(car_s+30,(2+4*lane),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			vector<double> next_x_60= getXY(car_s+60,(2+4*lane),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			vector<double> next_x_90= getXY(car_s+90,(2+4*lane),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			ptsx.push_back(next_x_30[0]);
			ptsx.push_back(next_x_60[0]);
			ptsx.push_back(next_x_90[0]);
						
			ptsy.push_back(next_x_30[1]);
			ptsy.push_back(next_x_60[1]);
			ptsy.push_back(next_x_90[1]);
			...
			double x_point_addon=0.0;

			for(int index=1;index<=50-previous_path_x.size();index++)
			{
				double N=(target_dist/(0.02*ref_vel/2.24));
				double x_point=x_point_addon+(target_dist)/N;
				double y_point=s(x_point);

				x_point_addon=x_point;
